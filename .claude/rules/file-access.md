...
func DecompressBoundedGzip(cfg DecompressionConfig, compressedSize int64, r io.Reader) ([]byte, error) {
    // Ratio guard before committing to decompression at all.
    ...
    limited := io.LimitReader(gz, ceiling+1) // Bounds the DECOMPRESSED side, not the input
    data, err := io.ReadAll(limited)
    if err != nil {
        return nil, err
    }
...
// not catch a bomb that expands gradually over many chunks.
func StreamDecompressBounded(cfg DecompressionConfig, r io.Reader, emit func([]byte) error) error {
    gz, err := gzip.NewReader(r)
    if err != nil {
    ...
    for {
        ...
        if n > 0 {
            ...
            if totalEmitted > cfg.MaxDecompressedBytes {
