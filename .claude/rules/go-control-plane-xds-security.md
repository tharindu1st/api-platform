...
xDS is itself a security control plane, not just a config-distribution mechanism: a spoofed or unauthenticated xDS channel can push arbitrary listener/cluster/route configuration to every data-plane instance it serves — including routes that bypass authn/authz, or upstreams that reach internal-only services. An unauthenticated Envoy admin interface allows both live traffic reconfiguration and TLS private-key exfiltration. Treat every directive below as being at the same severity tier as `authentication_authorization.md`, because a compromise here compromises everything that rule protects downstream.
...
```go
// BAD: plaintext, unauthenticated xDS gRPC server — by construction, not
// merely by a config toggle someone forgot to flip.
func NewEnvoyXDSServer() *grpc.Server {
    return grpc.NewServer() // No credentials.NewServerTLSFromFile at all
}

// BAD: TLS defaults to disabled for the policy xDS server, with no refusal
// to start when bound to a non-loopback address.
type PolicyServerTLSConfig struct {
    Enabled bool // Defaults false — the zero value is the insecure state
}

// BAD: stream callback logs the peer but never makes an accept/reject
// decision — any client that completes the TLS handshake gets the snapshot.
func (cb *serverCallbacks) OnStreamOpen(ctx context.Context, id int64, typ string) error {
    ...
    return nil // No identity check — this IS the authorization decision, and it always allows
}

// BAD: the downstream listener's private key is embedded directly in the LDS
// resource body as inline_bytes — a single LDS request exfiltrates it.
func createDownstreamTLSContext(cert, key []byte) *tlsv3.DownstreamTlsContext {
    return &tlsv3.DownstreamTlsContext{
        CommonTlsContext: &tlsv3.CommonTlsContext{
            TlsCertificates: []*tlsv3.TlsCertificate{{
                CertificateChain: &corev3.DataSource{Specifier: &corev3.DataSource_InlineBytes{InlineBytes: cert}},
                PrivateKey:       &corev3.DataSource{Specifier: &corev3.DataSource_InlineBytes{InlineBytes: key}}, // Exfiltrated via LDS
            }},
        },
    }
}
...
// BAD: no path normalization on the generated HCM — route/policy/authz
// matching runs against un-normalized paths.
func createListener() *listenerv3.Listener {
    hcm := &hcmv3.HttpConnectionManager{
        // NormalizePath, MergeSlashes, PathWithEscapedSlashesAction all left unset
    }
...
// construction — refuses to start otherwise unless bound to loopback.
func NewEnvoyXDSServer(cfg XDSServerConfig) (*grpc.Server, error) {
    if !cfg.TLS.Enabled && !isLoopback(cfg.BindAddr) {
