...
* **Cap Per-Request Resource Use Inside the Pool Too:** Where workers execute arbitrary or semi-trusted logic (a Python executor, a policy-engine script), apply a per-request memory/CPU ceiling (e.g. `resource.setrlimit` in a worker's init) in addition to bounding the number of concurrent workers — concurrency limits alone don't stop one request from exhausting resources by itself.

### 4. Add Jitter to Any Fixed-Interval Poll Against a Shared Upstream

* **Desynchronize Replica Polling:** A poll loop run identically across many replicas of the same service (an EventHub poll, a control-plane heartbeat) must desynchronize with random jitter — e.g. `sleep(interval + rand.Duration(0, interval/2))` — never a bare fixed-interval sleep. Without jitter, scale-out produces a synchronized thundering herd against the shared upstream every time all replicas restart or roll together.
...
```go
// BAD: bare ListenAndServe — no timeouts, no size caps, plaintext by default.
...
func NewXDSServer() *grpc.Server {
    return grpc.NewServer() // Defaults are effectively unbounded for this service's threat model
}

// BAD: unbounded queue in front of a bounded semaphore — the semaphore only
// limits how many run *concurrently*, not how many can be waiting.
var executionSlots = make(chan struct{}, 100)

func Execute(req *ExecRequest) error {
    executionSlots <- struct{}{} // Blocks forever if 100 are busy — callers queue without limit
    defer func() { <-executionSlots }()
    return run(req)
}

// BAD: no jitter — every replica polls in lockstep, hammering the shared
// upstream simultaneously on every restart/rollout.
...
// BAD: outer timeout is 12x the inner chain's own timeout — a stuck downstream
// call ties up the ext_proc worker for 60s when the chain itself gives up at 5s.
const MessageTimeout = 60 * time.Second // Policy-engine chain timeout is 5s

// BAD: a remote control plane's response directly kills the process — a
// hostile or malfunctioning control plane can take down every instance.
...
// GOOD: jittered polling — desynchronizes replicas instead of a thundering herd.
func PollEventHub(interval time.Duration) {
    for {
        fetchEvents()
