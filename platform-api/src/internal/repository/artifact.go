...
func (r *ArtifactRepo) GetAPIMetadataByHandle(handle, orgUUID string) (*model.APIMetadata, error) {
	...
	for i, e := range entries {
		parts[i] = fmt.Sprintf(
			"SELECT uuid, handle, name, version, '%s' AS type, organization_uuid FROM %s WHERE handle = ? AND organization_uuid = ?",
			e.KindAlias, e.Table,
		)
		args = append(args, handle, orgUUID)
	}
	query := "SELECT uuid, handle, name, version, type, organization_uuid FROM (\n\t\t\t" +
		strings.Join(parts, "\n\t\t\tUNION ALL\n\t\t\t") +
		"\n\t\t) combined"
...
func (r *ArtifactRepo) GetByHandle(handle, orgUUID string) (*model.Artifact, error) {
	...
	for i, e := range entries {
		parts[i] = fmt.Sprintf(
			"SELECT uuid, handle, name, version, '%s' AS type, organization_uuid, origin FROM %s WHERE handle = ? AND organization_uuid = ?",
			e.KindAlias, e.Table,
		)
		args = append(args, handle, orgUUID)
	}
	query := "SELECT uuid, handle, name, version, type, organization_uuid, origin FROM (\n\t\t\t" +
		strings.Join(parts, "\n\t\t\tUNION ALL\n\t\t\t") +
...
func (r *ArtifactRepo) GetByUUID(uuid, orgUUID string) (*model.Artifact, error) {
	...
	for i, e := range entries {
		parts[i] = fmt.Sprintf(
			"SELECT uuid, handle, name, version, '%s' AS type, organization_uuid, origin FROM %s WHERE uuid = ? AND organization_uuid = ?",
			e.KindAlias, e.Table,
		)
	...
	query := "SELECT uuid, handle, name, version, type, organization_uuid, origin FROM (\n\t\t\t" +
		strings.Join(parts, "\n\t\t\tUNION ALL\n\t\t\t") +
		"\n\t\t) combined " + r.db.FetchFirstClause(1)
